  ...
  const handlePointerUp = useCallback((e: React.PointerEvent<HTMLElement>) => {
    ...
    if (wasHorizontal) {
      ...
      if ((passedThreshold || isFastFlick) && deltaX > 0 && canNavigatePrev) {
        action = "prev";
      } else if ((passedThreshold || isFastFlick) && deltaX < 0 && canNavigateNext) {
        action = "next";
      }
      onDragEnd({ action });
    }
  }, [canNavigateNext, canNavigatePrev, onDragEnd, resetState, trackRef]);
